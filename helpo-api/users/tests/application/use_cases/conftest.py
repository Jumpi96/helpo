...
def user(exemplary_user_id: int, exemplary_user_email: str) -> User:
    return User(id=exemplary_user_id, email=exemplary_user_email, name="John Doe", user_type=1, is_staff=False, is_active=True, is_confirmed=True)
...
@pytest.fixture()
def volunteer_profiles_repo_mock() -> Mock:
    return Mock(spec_set=VolunteerProfilesRepository, delete=Mock())
